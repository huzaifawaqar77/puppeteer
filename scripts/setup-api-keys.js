...
async function setupApiKeysCollection() {
  ...
  await createCollectionWithAttributes(
    ...
    [
      ...
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "userId", 150, true),
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "keyHash", 150, true, undefined, undefined, true), // UNIQUE - encrypted string needs min 150
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "keyPrefix", 20, true),

      // Metadata (required)
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "name", 100, true),
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "description", 500, false),

      // Tier & Access Control (required)
      () => databases.createEnumAttribute(DATABASE_ID, "apiKeys", "tier", ["free", "premium"], true),
      () => databases.createEnumAttribute(DATABASE_ID, "apiKeys", "status", ["active", "inactive", "revoked", "expired"], true),

      // Usage Tracking (required)
      () => databases.createIntegerAttribute(DATABASE_ID, "apiKeys", "requestCount", true, 0),
      () => databases.createDatetimeAttribute(DATABASE_ID, "apiKeys", "lastUsedAt", false),

      // Rate Limiting (optional)
      () => databases.createIntegerAttribute(DATABASE_ID, "apiKeys", "dailyLimit", false),
      () => databases.createIntegerAttribute(DATABASE_ID, "apiKeys", "monthlyLimit", false),

      // Expiration & Security (required timestamps, optional expiration)
      () => databases.createDatetimeAttribute(DATABASE_ID, "apiKeys", "expiresAt", false),
      () => databases.createDatetimeAttribute(DATABASE_ID, "apiKeys", "createdAt", true),
      () => databases.createDatetimeAttribute(DATABASE_ID, "apiKeys", "updatedAt", true),

      // Permissions & Endpoints (required)
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "allowedEndpoints", 5000, true),
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "allowedOrigins", 5000, false),

      // Audit Trail (optional)
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "createdFromIP", 50, false),
      () => databases.createStringAttribute(DATABASE_ID, "apiKeys", "lastUsedFromIP", 50, false),
    ],
    [
      // Indexes
      () => databases.createIndex(DATABASE_ID, "apiKeys", "idx_userId", "key", ["userId"]),
      () => databases.createIndex(DATABASE_ID, "apiKeys", "idx_keyHash", "unique", ["keyHash"]),
      () => databases.createIndex(DATABASE_ID, "apiKeys", "idx_status", "key", ["status"]),
      () => databases.createIndex(DATABASE_ID, "apiKeys", "idx_expiresAt", "key", ["expiresAt"]),
      () => databases.createIndex(DATABASE_ID, "apiKeys", "idx_userId_status", "key", ["userId", "status"]),
      () => databases.createIndex(DATABASE_ID, "apiKeys", "idx_keyHash_status", "key", ["keyHash", "status"]),
    ]
  );
}
...
async function setupApiKeyFeatures() {
  ...
  try {
    ...
    console.log("  - status (Enum: active|inactive|revoked|expired) - Key status");
    ...
    console.error("  - Check API key has required scopes (databases.*.create_index, etc.)");
