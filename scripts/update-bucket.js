const { Client, Storage } = require("node-appwrite");

const APPWRITE_ENDPOINT = "https://appwrite.uiflexer.com/v1";
const APPWRITE_PROJECT_ID = "68c77b650020a2e5fa47";
const INPUT_BUCKET_ID = "input-files";
const OUTPUT_BUCKET_ID = "output-files";
const APPWRITE_API_KEY =
  process.env.APPWRITE_API_KEY ||
  "standard_";

const client = new Client()
  .setEndpoint(APPWRITE_ENDPOINT)
  .setProject(APPWRITE_PROJECT_ID)
  .setKey(APPWRITE_API_KEY);

const storage = new Storage(client);

async function updateBucket() {
  try {
    console.log("🔄 Updating input-files bucket...");

    // Update input bucket with new file extensions
    await storage.updateBucket(
      INPUT_BUCKET_ID,
      "Input Files",
      undefined, // permissions (keep existing)
      undefined, // fileSecurity
      true, // enabled
      30000000, // max file size (30MB)
      [
        "pdf",
        "jpg",
        "jpeg",
        "png",
        "doc",
        "docx",
        "xls",
        "xlsx",
        "ppt",
        "pptx",
        "odt",
        "ods",
        "odp",
        "rtf",
        "txt",
      ], // allowed extensions
      undefined, // compression
      true, // encryption
      false // antivirus
    );

    console.log("✅ Input bucket updated successfully!");

    console.log("🔄 Updating output-files bucket...");

    // Update output bucket with new file extensions
    await storage.updateBucket(
      OUTPUT_BUCKET_ID,
      "Output Files",
      undefined, // permissions (keep existing)
      undefined, // fileSecurity
      true, // enabled
      30000000, // max file size (30MB)
      [
        "pdf",
        "jpg",
        "jpeg",
        "png",
        "zip",
        "txt",
        "json",
        "html",
        "csv",
        "xml",
        "doc",
        "docx",
        "xls",
        "xlsx",
        "ppt",
        "pptx",
        "rtf",
      ], // allowed extensions
      undefined, // compression
      true, // encryption
      false // antivirus
    );

    console.log("✅ Output bucket updated successfully!");
    console.log(
      "📄 Output Allowed extensions: pdf, jpg, jpeg, png, zip, txt, json, html, csv, xml, doc, docx, xls, xlsx, ppt, pptx, rtf"
    );
  } catch (error) {
    console.error("❌ Error updating bucket:", error.message);
  }
}

updateBucket();
