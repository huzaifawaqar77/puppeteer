  ...
  try {
    ...
    if (attributes.length === 0) {
      ...
      attributes.forEach((attr, idx) => {
        ...
        console.log(`  ${idx + 1}. ${attr.key} (${type}) - ${required} ${unique}`);
      });
    }
    ...
    if (indexes.length === 0) {
      ...
      indexes.forEach((idx, i) => {
        ...
        console.log(`  ${i + 1}. ${idx.key} (Type: ${idx.type}, Attributes: ${keys})`);
      });
    }
    ...
    const criticalAttrs = ['userId', 'keyHash', 'keyPrefix', 'name', 'tier', 'status', 'requestCount'];
    const missingAttrs = criticalAttrs.filter(attr => !attributes.find(a => a.key === attr));
    ...
    const missingIndexes = criticalIndexes.filter(idx => !indexes.find(i => i.key === idx));
