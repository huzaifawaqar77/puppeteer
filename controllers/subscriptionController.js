...
function parseFeatures(featuresString) {
  ...
  if (typeof featuresString === "string") {
    try {
      ...
      console.error("Features string:", featuresString);
      return [];
    }
  }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        usage: {
          ...
          percentage_used:
            subscription.monthly_conversions > 0
              ? Math.round(
                  ((usage.length > 0 ? usage[0].usage_count : 0) /
                    subscription.monthly_conversions) *
                    100
                )
              : 0,
        },
  ...
  try {
    ...
    res.status(200).json({
      ...
      message: `Successfully ${
        newPlan.price === 0 ? "activated" : "upgraded to"
      } ${newPlan.name} plan!`,
      data: {
