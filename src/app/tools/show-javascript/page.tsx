...
export default function ShowJavascriptToolPage() {
  const { user } = useAuth();
  const [file, setFile] = useState<File | null>(null);
  const [processing, setProcessing] = useState(false);
  const [result, setResult] = useState<{ url: string; filename: string; textContent?: string } | null>(null);
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const job = await databases.createDocument(appwriteConfig.databaseId, appwriteConfig.collections.processingJobs, ID.unique(), {
        userId: user?.$id,
        operationType: "SHOW_JAVASCRIPT",
        status: "PENDING",
        inputFileIds: JSON.stringify([uploadedFile.$id]),
        startedAt: new Date().toISOString(),
      });

      const response = await fetch("/api/show-javascript", {
        method: "POST",
      ...
      if (!response.ok) throw new Error(data.error || "Failed to extract JavaScript");
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto max-w-4xl py-8 px-4">
      <div className="space-y-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Show JavaScript</h1>
          <p className="text-secondary">Extract and view embedded JavaScript code from PDF files</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-card">
          ...
          {result && (
            <div className="mb-6 space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                <h3 className="text-lg font-semibold text-green-900 mb-4">Extraction Complete!</h3>
              ...
              {result.textContent && (
                <div className="bg-slate-900 rounded-lg p-4 overflow-hidden">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Preview</span>
                  </div>
                  <pre className="text-slate-100 font-mono text-sm overflow-x-auto whitespace-pre-wrap max-h-96">
                    {result.textContent || "// No JavaScript found in this PDF"}
                  </pre>
                </div>
              )}
            </div>
          ...
          <button onClick={handleConvert} disabled={!file || processing} className="w-full bg-primary hover:bg-primary/90 disabled:bg-secondary/30 disabled:cursor-not-allowed text-white font-semibold py-3.5 px-6 rounded-lg transition-all hover:shadow-glow-orange flex items-center justify-center gap-2">
            {processing ? (<><Loader2 className="h-5 w-5 animate-spin" />Extracting...</>) : (<><Code className="h-5 w-5" />Extract JavaScript</>)}
          </button>
        </div>

        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-900"><strong>Security Tip:</strong> Malicious PDFs often contain hidden JavaScript. Use this tool to inspect suspicious files safely.</p>
        </div>
      </div>
    </div>
  );
