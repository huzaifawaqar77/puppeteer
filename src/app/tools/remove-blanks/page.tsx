...
import { Loader2, Download, FileMinus } from "lucide-react";

export default function RemoveBlanksToolPage() {
  const { user } = useAuth();
  ...
  const [whitePercent, setWhitePercent] = useState(99.0);
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const response = await fetch("/api/remove-blanks", {
        method: "POST",
  ...
  return (
    <div className="container mx-auto max-w-4xl py-8 px-4">
      <div className="space-y-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Remove Blank Pages</h1>
          <p className="text-secondary">Automatically detect and remove blank pages from your PDF documents</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-card">
          ...
          {file && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Sensitivity Threshold ({threshold})</label>
                <input
                  type="range"
                  min="1"
                  max="100"
                ...
                />
                <p className="text-xs text-secondary mt-1">Lower values detect more subtle content.</p>
              </div>
              
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">White Percentage ({whitePercent}%)</label>
                <input
                  type="range"
                  min="90"
                  max="100"
                  step="0.1"
                  value={whitePercent}
                  onChange={(e) => setWhitePercent(parseFloat(e.target.value))}
                  className="w-full h-2 bg-secondary/20 rounded-lg appearance-none cursor-pointer accent-primary"
                />
                <p className="text-xs text-secondary mt-1">Percentage of page that must be white to be considered blank.</p>
              </div>
            </div>
          )}
          ...
          <button onClick={handleConvert} disabled={!file || processing} className="w-full bg-primary hover:bg-primary/90 disabled:bg-secondary/30 disabled:cursor-not-allowed text-white font-semibold py-3.5 px-6 rounded-lg transition-all hover:shadow-glow-orange flex items-center justify-center gap-2">
            {processing ? (<><Loader2 className="h-5 w-5 animate-spin" />Processing...</>) : (<><FileMinus className="h-5 w-5" />Remove Blanks</>)}
          </button>
        </div>

        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-900"><strong>Tip:</strong> If pages with very little content are being removed, try increasing the sensitivity threshold or decreasing the white percentage.</p>
        </div>
      </div>
    </div>
  );
