...
export default function PdfToPresentationToolPage() {
  const { user } = useAuth();
  const [file, setFile] = useState<File | null>(null);
  const [processing, setProcessing] = useState(false);
  const [result, setResult] = useState<{ url: string; filename: string } | null>(null);
  const [error, setError] = useState<string>("");
  const [outputFormat, setOutputFormat] = useState<"ppt" | "pptx">("pptx");
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const job = await databases.createDocument(appwriteConfig.databaseId, appwriteConfig.collections.processingJobs, ID.unique(), {
        userId: user?.$id,
        operationType: "PDF_TO_PRESENTATION",
        status: "PENDING",
        inputFileIds: JSON.stringify([uploadedFile.$id]),
        startedAt: new Date().toISOString(),
      });

      const response = await fetch("/api/pdf-to-presentation", {
        method: "POST",
  ...
  return (
    <div className="container mx-auto max-w-4xl py-8 px-4">
      <div className="space-y-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">PDF to Presentation</h1>
          <p className="text-secondary">Convert PDF documents to PowerPoint presentations</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-card">
          ...
          <button onClick={handleConvert} disabled={!file || processing} className="w-full bg-primary hover:bg-primary/90 disabled:bg-secondary/30 disabled:cursor-not-allowed text-white font-semibold py-3.5 px-6 rounded-lg transition-all hover:shadow-glow-orange flex items-center justify-center gap-2">
            {processing ? (<><Loader2 className="h-5 w-5 animate-spin" />Converting...</>) : (<><Presentation className="h-5 w-5" />Convert to PowerPoint</>)}
          </button>
        </div>

        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-900"><strong>Tip:</strong> Complex layouts may require some adjustments in PowerPoint after conversion.</p>
        </div>
      </div>
    </div>
  );
