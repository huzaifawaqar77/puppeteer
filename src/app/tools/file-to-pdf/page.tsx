...
export default function FileToPdfToolPage() {
  const { user } = useAuth();
  ...
  async function handleConvert() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Failed to convert file");
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto max-w-4xl py-8 px-4">
      <div className="space-y-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Office to PDF</h1>
          <p className="text-secondary">Convert Word, Excel, PowerPoint, and other documents to PDF</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-card">
          ...
          <button onClick={handleConvert} disabled={!file || processing} className="w-full bg-primary hover:bg-primary/90 disabled:bg-secondary/30 disabled:cursor-not-allowed text-white font-semibold py-3.5 px-6 rounded-lg transition-all hover:shadow-glow-orange flex items-center justify-center gap-2">
            {processing ? (<><Loader2 className="h-5 w-5 animate-spin" />Converting...</>) : (<><FileType className="h-5 w-5" />Convert to PDF</>)}
          </button>
        </div>

        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-900"><strong>Supported Formats:</strong> Word (DOC, DOCX), Excel (XLS, XLSX), PowerPoint (PPT, PPTX), OpenOffice (ODT, ODP, ODS), RTF, and TXT.</p>
        </div>
      </div>
    </div>
  );
