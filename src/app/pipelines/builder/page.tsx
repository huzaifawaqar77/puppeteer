...
const AVAILABLE_OPERATIONS = [
  ...
  },
  {
    ...
    params: [
      { name: "splitType", type: "select", options: ["all", "pages"], default: "all", label: "Split Type" },
      { name: "pages", type: "text", default: "", label: "Pages (e.g. 1,3-5)", showIf: { param: "splitType", value: "pages" } }
    ]
  },
  {
    id: "rotate",
    ...
    params: [
      { name: "angle", type: "select", options: ["90", "180", "270"], default: "90", label: "Rotation Angle" }
    ]
  },
  {
    id: "compress",
    ...
    params: [
      { name: "compressionLevel", type: "select", options: ["1", "2", "3"], default: "2", label: "Compression Level (1=Low, 3=High)" }
    ]
  },
  {
    id: "protect",
  ...
  },
  {
    ...
    params: [
      { name: "text", type: "text", default: "CONFIDENTIAL", label: "Watermark Text" },
      { name: "rotation", type: "number", default: "45", label: "Rotation" },
      { name: "opacity", type: "number", default: "0.5", label: "Opacity (0-1)" }
    ]
  },
  {
    id: "auto-rename",
    ...
    params: [
      { name: "useFirstTextAsFallback", type: "boolean", default: false, label: "Use First Text as Fallback" }
    ]
  },
  {
    id: "remove-blanks",
    ...
    params: [
      ...
      { name: "whitePercent", type: "number", default: "99.0", label: "White Percentage" }
    ]
  },
  {
    id: "pdf-to-pdfa",
    ...
    params: [
      { name: "outputFormat", type: "select", options: ["pdfa-1b", "pdfa-2b", "pdfa-3b"], default: "pdfa-2b", label: "PDF/A Standard" }
    ]
  }
];
...
export default function PipelineBuilderPage() {
  ...
  const [selectedOperationId, setSelectedOperationId] = useState(AVAILABLE_OPERATIONS[0].id);
  ...
  const handleAddStep = () => {
    const operation = AVAILABLE_OPERATIONS.find(op => op.id === selectedOperationId);
    if (!operation) return;
  ...
  const handleUpdateParams = (stepId: string, paramName: string, value: any) => {
  ...
  const handleExecutePipeline = async () => {
    ...
    try {
      ...
      const pipelineJson = steps.map(step => {
        const operation = AVAILABLE_OPERATIONS.find(op => op.id === step.operationId);
        return {
  ...
  const getOperationDetails = (opId: string) => AVAILABLE_OPERATIONS.find(op => op.id === opId);

  return (
    <div className="container mx-auto max-w-6xl py-8 px-4">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2 flex items-center gap-3">
          <span className="bg-gradient-to-r from-amber-500 to-orange-600 text-transparent bg-clip-text">Visual Pipeline Builder</span>
          <span className="text-xs bg-amber-100 text-amber-800 px-2 py-1 rounded-full border border-amber-200">BETA</span>
        </h1>
        <p className="text-secondary">Chain multiple PDF operations together to create powerful automated workflows.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-card">
            ...
            <FileUploader onFilesSelected={setFiles} accept={[".pdf"]} multiple={true} maxSize={50 * 1024 * 1024} />
            {files.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {files.map((f, i) => (
                  <span key={i} className="text-xs bg-secondary/10 text-foreground px-2 py-1 rounded border border-border">
                    {f.name}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-card">
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-foreground mb-2">Pipeline Name</label>
              <input
                type="text"
                value={pipelineName}
            ...
            <div className="space-y-3">
              {steps.length === 0 ? (
                <div className="text-center py-12 border-2 border-dashed border-border rounded-xl bg-secondary/5">
                  <p className="text-secondary">No operations added yet. Add a step to start building.</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-medium text-foreground">{opDetails?.name}</h3>
                            <p className="text-xs text-secondary">{opDetails?.description}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button 
                            ...
                            className={`p-2 rounded-lg transition-colors ${isEditing ? 'bg-primary/20 text-primary' : 'hover:bg-secondary/10'}`}
                          >
                              ...
                              <div key={param.name}>
                                ...
                                {param.type === 'select' ? (
                                  <select
                                    ...
                                    onChange={(e) => handleUpdateParams(step.id, param.name, e.target.value)}
                                  ...
                                  <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                      ...
                                      onChange={(e) => handleUpdateParams(step.id, param.name, e.target.checked)}
