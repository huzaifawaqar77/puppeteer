...
export async function POST(req: NextRequest) {
  try {
    const { fileId, jobId, outputFormat } = await req.json();
    ...
    const response = await fetch(`${stirlingUrl}/api/v1/convert/pdf-to-text`, {
      method: "POST",
      headers: {
    ...
    const processedFile = await storage.createFile(
      ...
      ID.unique(),
      InputFile.fromBuffer(Buffer.from(processedBuffer), `converted.${extension}`)
    );
