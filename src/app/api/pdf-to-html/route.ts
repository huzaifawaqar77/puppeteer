...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3. Call Stirling PDF API (correct endpoint: /pdf/html not /pdf-to-html)
    ...
    const response = await fetch(`${stirlingUrl}/api/v1/convert/pdf/html`, {
      method: "POST",
      headers: {
    ...
    const processedFile = await storage.createFile(
      ...
      ID.unique(),
      InputFile.fromBuffer(Buffer.from(processedBuffer), "converted.html")
    );
