...
export async function POST(request: NextRequest) {
  try {
    const { fileId, jobId, threshold = "10", whitePercent = "99.0" } = await request.json();
    ...
    const stirlingResponse = await fetch(
      `${stirlingConfig.url}/api/v1/misc/remove-blanks`,
      {
        method: "POST",
        headers: {
    ...
    const processedFile = new File([processedBlob], "cleaned.pdf", {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to remove blank pages" },
      { status: 500 }
    );
  }
}
