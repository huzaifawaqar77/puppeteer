...
export async function POST(req: NextRequest) {
  try {
    ...
    formData.append("flattenOnlyForms", "true"); // Required: true=flatten forms only, false=convert to images

    // 3. Call Stirling PDF API (correct endpoint: /misc/flatten not /general/flatten)
    ...
    const response = await fetch(`${stirlingUrl}/api/v1/misc/flatten`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error("❌ Stirling PDF flatten error:");
    ...
    const processedFile = await storage.createFile(
      ...
      ID.unique(),
      InputFile.fromBuffer(Buffer.from(processedBuffer), "flattened.pdf")
    );
    ...
    return NextResponse.json(
      { error: error.message || "Failed to flatten PDF" },
      { status: 500 }
    );
  }
}
