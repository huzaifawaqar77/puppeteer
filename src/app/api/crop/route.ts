...
export async function POST(req: NextRequest) {
  try {
    const { fileId, jobId, coordinates } = await req.json();

    if (!fileId || !jobId || !coordinates) {
    ...
    formData.append("removeDataOutsideCrop", "true"); // Remove text outside crop area
    ...
    const response = await fetch(`${stirlingUrl}/api/v1/general/crop`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error("❌ Stirling PDF crop error:");
    ...
    const processedFile = await storage.createFile(
      ...
      ID.unique(),
      InputFile.fromBuffer(Buffer.from(processedBuffer), "cropped.pdf")
    );
