...
export async function POST(req: NextRequest) {
  try {
    const { fileId, jobId, language } = await req.json();
    ...
    const response = await fetch(`${stirlingUrl}/api/v1/misc/ocr-pdf`, {
      method: "POST",
      headers: {
    ...
    const processedFile = await storage.createFile(
      ...
      ID.unique(),
      InputFile.fromBuffer(Buffer.from(processedBuffer), "ocr_processed.pdf")
    );
