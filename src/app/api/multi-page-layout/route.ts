...
export async function POST(req: NextRequest) {
  try {
    const { fileId, jobId, pagesPerSheet } = await req.json();
    ...
    // Stirling PDF might expect 'nUp' or similar parameter.
    // Based on common endpoints: /api/v1/general/n-up-pdf
    // Parameters usually include 'nUp' (int)
    formData.append("nUp", pagesPerSheet || "2");
    ...
    const response = await fetch(`${stirlingUrl}/api/v1/general/n-up-pdf`, {
      method: "POST",
      headers: {
    ...
    const processedFile = await storage.createFile(
      ...
      ID.unique(),
      InputFile.fromBuffer(Buffer.from(processedBuffer), "layout.pdf")
    );
    ...
    console.error("Multi-Page Layout error:", error);
    return NextResponse.json(
      { error: error.message || "Failed to create multi-page layout" },
      { status: 500 }
    );
  }
}
