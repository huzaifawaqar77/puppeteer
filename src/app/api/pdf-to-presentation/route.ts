...
export async function POST(request: NextRequest) {
  try {
    const { fileId, jobId, outputFormat = "pptx" } = await request.json();
    ...
    const stirlingResponse = await fetch(
      `${stirlingConfig.url}/api/v1/convert/pdf/presentation`,
      {
        method: "POST",
        headers: {
    ...
    const processedFile = new File([processedBlob], `converted.${outputFormat}`, {
    ...
    console.error("PDF to Presentation error:", error);
    return NextResponse.json(
      { error: error.message || "Failed to convert PDF to Presentation" },
      { status: 500 }
    );
  }
}
