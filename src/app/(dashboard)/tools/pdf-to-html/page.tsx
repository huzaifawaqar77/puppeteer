...
export default function PdfToHtmlToolPage() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Failed to convert PDF to HTML");
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto max-w-4xl py-8 px-4">
      <div className="space-y-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">PDF to HTML</h1>
          <p className="text-secondary">Convert PDF documents to HTML format</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-card">
          ...
          {result && (
            <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-green-900 mb-4">HTML Generated!</h3>
          ...
          <button onClick={handleConvert} disabled={!file || processing} className="w-full bg-primary hover:bg-primary/90 disabled:bg-secondary/30 disabled:cursor-not-allowed text-white font-semibold py-3.5 px-6 rounded-lg transition-all hover:shadow-glow-orange flex items-center justify-center gap-2">
            {processing ? (<><Loader2 className="h-5 w-5 animate-spin" />Converting...</>) : (<><Globe className="h-5 w-5" />Convert to HTML</>)}
          </button>
        </div>

        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-900"><strong>Tip:</strong> HTML output preserves text and formatting for web display.</p>
        </div>
      </div>
    </div>
  );
